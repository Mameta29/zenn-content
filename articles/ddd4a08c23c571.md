...
- **対象ユーザー**: 業務委託・副業ワーカーへの発注が多く、月末の請求書処理・送金・記帳に消耗している中小企業のバックオフィス／経営者／エンジニア。特に Microsoft 365（Teams / Entra ID / Fabric）をすでに業務で使っている組織。
- **課題**: 取引先が増えるほど月末経理が掛け算で重くなる。請求書の手入力、銀行の振込フォーム入力、海外送金で着金まで3〜5日、手数料は1件数千円、フリーランサーからの催促対応 ── 誰も得しない待ち時間とコストが積み上がる。
- **ソリューション**: 発注 → 検収 → 送金 → 記帳までを4つの AI エージェント（Contract / Review / Settlement / Bookkeeping）で自律的に回し、**PR がマージされた約3秒後に JPYC（円建てステーブルコイン）で報酬が着金する** システム「Agentic Gig-Flow」を作った。Copilot Studio / Azure Functions / Foundry（Azure OpenAI）/ Fabric / Entra ID を組み合わせ、PM・Worker・経理・経営者の4人がそれぞれ別の入口から同一の自律エンジンに繋がる構造になっている。
:::
...
- [Mameta29/agentic-gig-flow](https://github.com/Mameta29/agentic-gig-flow) … 本体（Functions / Dashboard / Copilot Studio 連携）
- [Mameta29/gigflow-demo-workspace](https://github.com/Mameta29/gigflow-demo-workspace) … デモ用のワーカー側リポジトリ
