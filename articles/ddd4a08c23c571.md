---
title: "PRをマージした3秒後に、円が届く ── AIエージェント × JPYC × Microsoft で中小企業の月末経理を消した話"
emoji: "💸"
...
副業・業務委託で働く人は、日本だけで3,000万人規模と言われています。発注側の中小企業からすると、これは「取引先が一気に増える」ということでもあります。そして取引先が増えるほど大変なのが、**月末のバックオフィス業務**です。
...
▶ 実際の発注シーンは [デモ動画](https://youtu.be/V4e1St7Wb3c)（00:00〜）で確認できます。
...
▶ PR 作成から自動マージまでの流れは [デモ動画](https://youtu.be/V4e1St7Wb3c?t=96)（01:36〜）で確認できます。
...
▶ マージ直後に JPYC が着金する瞬間は [デモ動画](https://youtu.be/V4e1St7Wb3c?t=104)（01:44〜）で確認できます。
...
Review Agent で一番効果があったのが、**判定の根拠を必ず引用させる**ことです。[4.2](#4.2-review-agent) で出したルールの再掲ですが、System Prompt で「acceptanceCriteria 各項目について、diff から **証拠（ファイルパス + 抜粋）** を引用しながら met / not_met を判定する。推測でなく diff から確認できる事実のみ」と縛っています。
...
LLM のツールリストは、「LLM の判断材料を取りに行くツール」と「LLM の判断結果を外に出力するツール」までに留めるのがよくて、**「LLM の判断のあとに来る、決定的な順序付き副作用」はサーバ側に置く**のが、個人的にはバランスが良いと思っています。
...
の2軸で考える、という整理になります。派手に AI に色々やらせるよりも、この2つの境界を慎重に引くほうが、実運用に乗るシステムにはずっと重要だなと、今回の race condition で実感しました。
